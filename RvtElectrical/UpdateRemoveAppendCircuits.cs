...
namespace RvtElectrical
{
    ...
    public class UpdateRemoveAppendCircuits : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            //Get Powered Power Connector Circuits that are Appended
            List<Element> powerCircuits = new List<Element>();
            ElectricalSystemSet ess = new ElectricalSystemSet();
            List<ElectricalCircuitData> powerCircuitData = new List<ElectricalCircuitData>();

            List<DeviceConnector> tempPowerConnectors = new List<DeviceConnector>();
            foreach (var powerConnector in powerConnectors)
            {
                if (powerConnector.CircuitData.HasCircuit && powerConnector.CircuitData.DeviceElectricalSystem.Elements.Size>1)
                {
                    tempPowerConnectors.Add(powerConnector);
                    
                    //powerCircuits.Add(powerConnector.CircuitData.CircuitElement);
                    //powerCircuitData.Add(powerConnector.CircuitData);
                    //ess.Insert(powerConnector.CircuitData.DeviceElectricalSystem);
                }
            }

            powerConnectors.Clear();
            powerConnectors = tempPowerConnectors;

            //Get Distinct Circuit Number
            List<string> discreteCircuits = new List<string>();
            foreach(var powerConnector in powerConnectors)
            {
                discreteCircuits.Add(powerConnector.CircuitData.DeviceElectricalSystem.CircuitNumber);
            }
            discreteCircuits = discreteCircuits.Distinct().ToList();


            foreach(var discreteCircuit in discreteCircuits)
            {
                ElementSet esCircuits = new ElementSet();
                ElectricalCircuitData testCircuit = null;
                foreach(var powerConnector in powerConnectors)
                {
                    if(powerConnector.CircuitData.DeviceElectricalSystem.CircuitNumber == discreteCircuit)
                    {
                        if(testCircuit == null)
                        {
                            testCircuit = powerConnector.CircuitData;
                        }
                        else
                            esCircuits.Insert(powerConnector.CircuitData.CircuitElement);
                    }
                }
                testCircuit.RemoveElementFromCircuit(esCircuits);
            ...
            //Element selectedPanelElement = selectedPanel.Panel;
