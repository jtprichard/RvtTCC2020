...
{
    static class RibbonPanels
    //Ribbon Panel and Button Creation
    {
        //DEVICE BOX RIBBON PANEL
        public static void RibbonPanelDeviceBox(UIControlledApplication application, string path, string tabName)
        {
            RibbonPanel panel = application.CreateRibbonPanel(tabName, "Performance Electrical");

            //BUTTON TO UPDATE DEVICE SCHEDULES
            PushButtonData button = new PushButtonData("Update_Device_Schedules_Button", "Update Device\nSchedules", 
                path, "RvtElectrical.CmdUpdateDeviceSchedules")
            {
                ToolTip = "Update the circuit numbers on the sheet device schedules"
            };
